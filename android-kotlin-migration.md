...
Kotlin developers provide an [online Java-to-Kotlkin transpiler](https://try.kotlinlang.org/). It is far from perfect, but it is enough for beginers with an existing Java project. In our application source directory, we create the same architecture where we replace `src/main/java/<package>` with `src/main/kotlin/<package>`. We transpile all the `.java` files to `.kt` and we run again our ` gradlew` script.
